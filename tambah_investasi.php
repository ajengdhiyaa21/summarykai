...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $jumlah_dana = $_POST['jumlah_dana']?? 0;// Hitung jumlah_dana
    ...
    $input_date = $_POST['input_date'] ?? date('Y-m-d');
    ...
    if (empty($errors)) {
        // Pastikan pernyataan SQL benar
        $stmt = $conn->prepare("INSERT INTO investasi (no, uraian, wbs, lokasi_pengadaan, volume_satuan, harga_satuan, jumlah_dana, budget_tahun_2024, tambahan_dana, total_tahun_2024, commitment, actual, consumed_budget, available_budget, progres_saat_ini, tanggal_kontrak, no_kontrak, nilai_kontrak, ket, input_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        // Bind parameters
        $stmt->bind_param("isss sdddd dddddds sds s",
            $no,
            $uraian,
            $wbs,
            $lokasi_pengadaan,
        ...
        header("Location: investasi.php"); // Redirect to investasi.php after successful insert
        exit;
    }
}
?>
    ...
    <div class="card p-3" style="padding: 30px; margin-bottom: 20px;">
        ...
        <form method="POST" action="" novalidate>
            <div class="mb-3">
                ...
                <input type="date" class="form-control" id="input_date" name="input_date" value="<?= htmlspecialchars($_POST['input_date'] ?? date('Y-m-d')) ?>">
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" step="0.01" class="form-control bg-light" id="jumlah_dana" name="jumlah_dana" value="<?= htmlspecialchars($jumlah_dana ?? '') ?>" readonly>
            </div>
            <div class="mb-3">
